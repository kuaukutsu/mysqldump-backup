...
class Sendmail(object):
    ...
    def run(self):
        self._logger.info('sendmail run')

        filesize = os.path.getsize(self._filepath)
        self._logger.info('backup size {0}'.format(filesize))
        ...
        if 0 < self._config.email_chunk_max_size < filesize:
            ...
            for f in os.listdir(dir_split):
                if os.path.isfile(os.path.join(dir_split, f)):
                    ...
                    getattr(self, 'send_{}'.format(self._config.email_transport))(filepath)
                    os.remove(filepath)

            # clear directory
            shutil.rmtree(dir_split)
        else:
            getattr(self, 'send_{}'.format(self._config.email_transport))(self._filepath)

    def send_smtp(self, filepath):
        Smtp(self._config).send(filepath)

    def send_api(self, filepath):
        Mailgun(self._config).send(filepath)
